...
from lab_devices.experiment.analyze import ScalarType, assignable, infer_type
from lab_devices.experiment.expr import parse_expression


def _t(text: str, binds: dict[str, str] | None = None):
    bt = {k: ScalarType(v) for k, v in (binds or {}).items()}  # base-only binds for these tests
...
def test_integer_literal_and_count_are_int() -> None:
    assert _t("5").type.base == "int"
    assert _t("count(s)").type.base == "int"
    assert _t("5.0").type.base == "number"


def test_int_arithmetic_stays_int_but_division_and_float_widen() -> None:
    assert _t("2 + 3").type.base == "int"
    assert _t("2 * 3").type.base == "int"
    assert _t("2 / 3").type.base == "number"  # real division always widens
    assert _t("2 + 3.0").type.base == "number"
    assert _t("-2").type.base == "int"  # unary minus preserves int


def test_stat_over_stream_is_number() -> None:
    assert _t("mean(s)").type.base == "number"
    assert _t("last(s)").type.base == "number"


def test_string_binding_flows_as_string_without_a_problem() -> None:
    r = _t("mode", {"mode": "string"})
    assert r.type.base == "string"
    assert r.problems == ()


def test_string_equality_is_allowed() -> None:
    r = _t("mode == mode", {"mode": "string"})
    assert r.type.base == "bool"
    assert r.problems == ()
