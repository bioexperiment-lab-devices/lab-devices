...
async def test_escaping_artifact_dir_is_refused(api: SimpleNamespace) -> None:
    """Row doctored to point outside data_dir -> 404 on all artifact readers, delete
    keeps row-removal semantics but never touches the outside path."""
    record = await _seed_record(api)
    record_id = record["id"]
    ...
    for path in (
        f"/api/records/{record_id}/events",
