...
/** Labs tab (spec §9.2): lab picker with online badges, read-only device table (including
 * the operator-chosen name), and a confirmed Rediscover that re-enumerates the lab's bus.
 * Naming and device control live in the Devices tab; the Name column here is display-only. */
              ...
              <div className={s.discovering ? 'pointer-events-none opacity-50' : ''}>
                <table className="w-full border-collapse rounded bg-white text-sm">
                  ...
                  <tbody>
                    {s.devices.length === 0 && (
                      <tr>
                        <td colSpan={7} className="px-2 py-4 text-center text-xs text-hint">
                          no devices attached
                        </td>
                      </tr>
                    )}
