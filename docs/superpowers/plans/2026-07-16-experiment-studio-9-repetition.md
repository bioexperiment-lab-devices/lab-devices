...
**Carried forward from Task 2's review (compound `->` paths).** A diagnostic path may be compound: `blocks[0].body[0]->mygroup.body[0] compute value`. This arises whenever a validate-phase walk crosses from a `group_ref` call site into a **plain** (non-parametrized) group's body (`validate.py:894`, `:940`) — `_remap` (Task 2) now maps only the call-site segment before the first `->` and carries everything from `->` onward untouched, since that tail already names an authored location inside the group definition. The **authored, editable** location for such a diagnostic is **inside the group definition** — i.e. it should resolve to `groups['mygroup'].body[0]` and switch the canvas to the `mygroup` scope, which is where the author can actually fix it. The call-site prefix (`blocks[0].body[0]`) is context, not the edit target. This is a **second, independent** parsing requirement on `resolveDiagnosticPath` alongside the quote-style caveat below (Task 1's review) — a single path may need both a `->` split and a quote-tolerant `groups[...]` match on either side of it.
