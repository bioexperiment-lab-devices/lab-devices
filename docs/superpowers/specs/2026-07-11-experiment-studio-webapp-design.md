...
| `POST /api/experiments/import` | create from an uploaded doc, auto-renaming `(copy)`/`(copy 2)`… on name conflict; **never 409s**; no `validate_doc` gate — import is a save, not a run (amended 2026-07-16 during W7: export/import needed a create path that never conflicts, see the companion spec `docs/superpowers/specs/2026-07-16-experiment-studio-export-import-design.md`) |
...
| `POST /api/runs/{id}/pause` / `resume` / `abort` | engine `pause()/resume()/abort()`; 404 if not the active run; abort is idempotent while active (amended 2026-07-12 during W6: once the run records a terminal status — including the brief finalization window — controls 404 like any non-active run; the frontend swallows these by design) |
...
resume); YAML/DSL import-export (amended 2026-07-16 during W7: JSON file export/import
shipped — `docs/superpowers/specs/2026-07-16-experiment-studio-export-import-design.md`;
YAML or a text DSL is still deferred); Blockly-style keyboard-driven block entry; record
retention policies.
